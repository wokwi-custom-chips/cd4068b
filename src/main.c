...
static void set_logic(chip_state_t *chip) {
  subGate1 = pin_read(chip->pin_A) & pin_read(chip->pin_B) ;
  subGate2 = pin_read(chip->pin_C) & pin_read(chip->pin_D);
  subGate3 = pin_read(chip->pin_E) & pin_read(chip->pin_F) ;
  subGate4 = pin_read(chip->pin_G) & pin_read(chip->pin_H) ;
  pin_write(chip->pin_J, !( subGate1 &  subGate2 & subGate3 & subGate4 ) );
  pin_write(chip->pin_K, ( subGate1 &  subGate2 & subGate3 & subGate4 ) );
}
...
static void chip_pin_change(void *user_data, pin_t pin, uint32_t value) {
  ...
  //printf("Gate 3 :  %d\n" , subGate3);
  //printf("Gate 4 :  %d\n" , subGate4);
  //printf("Output State J :  %u\n" , pin_read(chip->pin_J));
  //printf("Output State K :  %u\n" , pin_read(chip->pin_K));
}


void chip_init() {
  printf("Initialize sn5454 chip!\n");
